...
dfs = []
for date in tqdm(df1['日期'].unique()):
    #遍历所有列，寻找包含“故障”的列
    faults_data = pd.DataFrame()
    faults_data = pd.concat([faults_data, pd.DataFrame({'日期': [date]})], axis=1)
    for column in df1.columns:
        if '故障' in column:
            #将DataFrame中特定列转换为Panda Series，只考虑日期为3的数据
            series = pd.Series(df1[df1['日期'] == date][column])
...
all_faults_df = pd.concat(dfs, axis=0, ignore_index=True).reset_index(drop=True)
all_faults_df['日期'].fillna(method='ffill',inplace=True)
print("test")

grouped_data = all_faults_df.groupby('日期').mean().reset_index()
...
grouped_data.rename(columns=column_mapping,inplace=True)
last_records = pd.concat([last_records,grouped_data],axis=1)
...
operator_info_df = pd.read_excel('操作人员信息表.xlsx')
print("test")
...
plt.title('Correlation between length of service and other variables')
